...
export default function ReservationWidget(props: { settings: any }) {
  ...
  >([]);
  const [selectedOccasionItems, setSelectedOccasionItems] = useState<
    Array<{
      uniqueKey: string;
      itemId: number;
      itemNameEn: string;
  ...
  const handleAddVariation = (item: any, variationGroup: any, valObj: any) => {
    const unitPrice =
      valObj.price !== null && valObj.price !== undefined && valObj.price !== ""
  ...
  const handleIncrement = (uniqueKey: string) => {
    setSelectedOccasionItems((prev) =>
      prev.map((i) =>
        i.uniqueKey === uniqueKey ? { ...i, count: i.count + 1 } : i,
      ),
    );
  };
  ...
  useEffect(() => {
    ...
    console.log(selectedOccasionItems);
    selectedOccasionItems.forEach((sel) => {
      const lineTotal = sel.unitPrice * sel.count;
      itemsPrice += lineTotal;
      const itemName =
        locale === "ar"
          ? sel.itemNameAr || sel.itemNameEn
          : sel.itemNameEn || sel.itemNameAr;
      let title = itemName;
      if (sel.hasVariations && (sel.variationValueEn || sel.variationValueAr)) {
        const varVal =
          locale === "ar"
            ? sel.variationValueAr || sel.variationValueEn
            : sel.variationValueEn || sel.variationValueAr;
    ...
    if (addVat) {
      ...
      const itemsTotalPrice =
        downPayment > 0
          ? itemsPrice + clacVat + downPayment
          : itemsPrice + clacVat;
      setTotalPrice(itemsTotalPrice);
      form.setValue("occasionItemsPrice", itemsTotalPrice);
    } else {
      const itemsTotalPrice =
        downPayment > 0 ? itemsPrice + downPayment : itemsPrice;
      setTotalPrice(itemsTotalPrice);
  ...
  useEffect(() => {
    ...
    if (settings.settings?.enable_booking_notice) {
      ...
      setTimeout(() => {
        bookingNotice.current?.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }, 100);
    ...
    let seatingT = 0;
    timeItem.duration[guests]
      ? (seatingT = timeItem.duration[guests])
      : (seatingT = 120); // default duration 2 hours
  ...
  function itemIsAvailable(item: any) {
    ...
    if (
      reservationDay - toDay >= item.reservation_availability_period ||
      reservationMonth > currentMonth
    ) {
      if (
        reservationDay == toDay &&
        item.available_before_time <= currentHour
      ) {
        return false;
      }
      return true;
    ...
    <div className="content w-[90vw] max-w-[800px] flex flex-col items-center justify-center my-[104px] gap-12 clg:my-5">
      <div className="theme-border bg-[#e5cbbd] flex flex-col gap-4 p-8 tablet:p-0 w-full reservation-widget relative">
        ...
        {showSummary && (
          <div
            ref={bookingForm}
            className="w-full flex flex-col gap-1 mb-4 animated zoomIn"
          >
        ...
        {showForm && (
          <Form {...form}>
            <form
              ...
              className="flex flex-col w-full gap-4 animated zoomIn"
            >
              {!showSummary && (
                <div className="gap-4 flex flex-col">
                  <div className="flex gap-4 tablet:flex-col">
                    ...
                    />
                    {specialDay && (
                      <div className="flex items-end font-Rufina text-xl pb-[4px] font-semibold">
                        <div className="flex items-center gap-4">
                          <CelebrationSymbol size={32} />
                          {specialDay[`name_${locale}`]}
                        </div>
                      </div>
                    )}
                  </div>
                          ...
                          <FormControl>
                            ...
                            >
                              {availability.length > 0 ? (
                                availability.map(
                                  (
                                    item: { time: string; payment?: number },
                                    index,
                                  ) => (
                                    <ToggleGroupItem
                                      key={index}
              ...
              {showTimer && (
                <div className="flex flex-col gap-4 tablet:flex-col justify-items-stretch animated zoomIn">
                  ...
                  {occasion === true && (
                    <div className="flex flex-col gap-2 border-b border-black boder-b-0">
                      ...
                      {settings.settings?.enable_occasion_items && (
                        <div className="pb-4">
                          <h4 className="py-4 text-base font-bold">
                            {
                              settings.settings[
                                `occasion_items_title_${locale}`
                              ]
                            }
                          </h4>
                            ...
                            {occasionItems.map(
                              (category: any, catIndex: number) =>
                                ...
                                category.items.length > 0 && (
                                  <div
                                    key={catIndex}
                                    className="flex flex-col w-full gap-4 pb-6 border-b border-mainColor"
                                  >
                                              ...
                                              <div className="w-full flex flex-row ss:flex-col justify-start gap-4">
                                                ...
                                                <div className="flex flex-col w-full justify-between">
                                                  ...
                                                  {item[
                                                    ...
                                                    <p
                                                      className={`text-sm text-gray-600 ${locale === "en" ? "text-left pt-2" : "text-right pt-2"}`}
                                                    >
                                                      {
                                                        item[
                                                          `description_${locale}`
                                                          ...
                                                          >
                                                            <div className="flex flex-col">
                                                              <span className="font-medium text-stone-800">
                                                                {locale === "ar"
                                                                  ? selItem.variationValueAr ||
                                                                    selItem.variationValueEn
                                                                  : selItem.variationValueEn ||
                                                                    selItem.variationValueAr}
                                                              </span>
                                                              <span className="text-xs text-stone-500 flex items-center gap-1">
                                                                ...
                                                                {selItem.count >
                                                                1
                                                                  ? `× ${selItem.count} = ${selItem.unitPrice * selItem.count}`
                                                                  : ""}
                                                              </span>
                                                            </div>
                                            ...
                                            {giftCards.map((card: any) => {
                                              ...
                                              return (
                                                <button
                                                  ...
                                                  className={`relative rounded-lg overflow-hidden border-2 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-mainColor ${
                                                    isSelected
                                                      ? "border-mainColor shadow-md"
                                                      : "border-transparent hover:border-mainColor/40"
                                                  }`}
                                                >
                                                  <Image
                                                    src={
                                                  ...
                                                  />
                                                  {isSelected && (
                                                    <div className="absolute top-2 end-2 bg-mainColor text-white rounded-full p-1 animated zoomIn">
                                                      <Check className="w-4 h-4" />
                                                    </div>
                                                  )}
                                                  {cardTitle && (
                                                    <div className="absolute bottom-0 inset-x-0 bg-black/50 text-white text-sm py-1 px-2 text-center">
                                                      {cardTitle}
                                                    </div>
                                                  )}
                                                </button>
              ...
              {(orderItems.length > 0 || downPayment > 0) && showTimer && (
                <div className="bg-white px-2 py-4 rounded-lg shadow-md flex-1 flex flex-col justify-center items-center text-center w-full px-6 mt-4">
                  ...
                  {addVat && vat > 0 && (
                    <PaymentItem
                      title={`${t("vat")} (${settings.settings.vat_value}%)`}
                      value={vat}
                    />
                  )}
                  {downPayment > 0 && (
        ...
        {loading && (
          <div className="">
            ...
            <div className="loader-container flex"></div>
          </div>
        )}
      </div>
    </div>
